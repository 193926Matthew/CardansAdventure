...
public class LobbyTileset extends Tileset {
    public LobbyTileset() {
        super(ImageLoader.load("LobbyTileset.png"), 48, 48, 1);
    }
    ...
    public ArrayList<MapTileBuilder> defineTiles() {
        ...
        // Sand Brick
        Frame SandBrickFrame = new FrameBuilder(getSubImage(4, 2))
                .withScale(tileScale)
                .build();
        MapTileBuilder SandBrickTile = new MapTileBuilder(SandBrickFrame)
                .withTileType(TileType.PASSABLE);

        mapTiles.add(SandBrickTile);

        // Snow block
        Frame SnowBlockFrame = new FrameBuilder(getSubImage(4, 3))
                .withScale(tileScale)
                .build();
        MapTileBuilder SnowBlockTile = new MapTileBuilder(SnowBlockFrame)
                .withTileType(TileType.PASSABLE);

        mapTiles.add(SnowBlockTile);

        //Passable tree block
        Frame PasstreeTrunkFrame = new FrameBuilder(getSubImage(1, 0))
                .withScale(tileScale)
                .build();

        MapTileBuilder PasstreeTrunkTile = new MapTileBuilder(PasstreeTrunkFrame)
                .withTileType(TileType.PASSABLE);

        mapTiles.add(PasstreeTrunkTile);
